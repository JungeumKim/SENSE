...
def parse_args():
    ...
    parser.add_argument('--cfg_path', default='configs/train_R_AT_capa.json', type=str,
    ...
    parser.add_argument('--attack_eps', default=None,type=float, help='')
    parser.add_argument('--attack_stepsize', default=None,type=float, help='')
...
def main(args):
    ...
    if configs.mode == 'train':
        ...
        configs.save_path = os.path.join(configs.save_path,F"capa{configs.capa}",["full","demo"][args.demo],configs.mode,configs.alg,F"warm{configs.warmup}",F"c{configs.lossCut}",F"nat_weight{configs.nat_weight}")
